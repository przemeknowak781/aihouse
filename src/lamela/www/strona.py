...
def _pominiete(R: dict, kadr: str) -> str:
    d = (R.get("pominiete") or {}).get(kadr) or []
    if not d:
        return ""
    return "W tym ujęciu pominięto drzewa z projektu zieleni, które zasłaniałyby bryłę: " + \
        ", ".join(f'{x["gatunek"].split("(")[0].strip()} ({x["id"]})' for x in d) + "."
...
def hero(D: dict, tr: dict, W: dict, R: dict, sep: str) -> str:
    ...
    return (f'<section class="hero wrap" id="top" aria-labelledby="h-nazwa">'
            ...
            f'<div class="hero-cap"><span>Widok od ogrodu, render z modelu 3D · {E(_slonce_txt(info))}. {E(_pominiete(R, "ogrod_169"))}</span>'
