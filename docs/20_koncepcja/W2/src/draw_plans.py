...
def stairs(ax, kond):
    ...
    if kond in ("P0", "P1"):
        ...
        ax.text((xw0 + xe1) / 2, (yl0 + yl1) / 2, f"spocznik\n+{D.fmt(M.POZ[kond] + 1.575, 3)}", fontsize=5.5, ha="center", va="center", zorder=6)
    if kond == "P0":
...
def furniture(ax, kond):
    ...
    if kond == "P0":
        ...
        f(ax, 0.5, 4.1, 3.2, 4.65, "RTV/regal", 4.5)
        f(ax, 5.0, 1.7, 7.4, 2.7, "stol 8 os."); f(ax, 8.90, 1.80, 10.70, 2.75, "wyspa")
        f(ax, 11.295, 2.05, 11.895, 4.695, "zabudowa", 4.5); f(ax, 10.30, 4.095, 11.295, 4.695, "", 4.5)
        f(ax, 0.40, 6.295, 1.80, 8.295, "lozko\n140"); f(ax, 3.17, 6.30, 3.77, 8.295, "szafa", 4.5)
        ...
        f(ax, 12.2, 0.15, 14.15, 0.55, "lawka / szafy", 4.2); f(ax, 14.40, 0.15, 15.35, 0.55, "polki", 4)
    if kond == "P1":
        f(ax, 0.3, 1.2, 1.9, 3.2, "lozko\n90x200", 4.5); f(ax, 2.2, 0.2, 3.7, 0.8, "biurko", 4.5); f(ax, 3.2, 1.4, 3.8, 3.4, "szafa", 4.5)
        f(ax, 0.3, 5.3, 1.9, 7.3, "lozko\n90x200", 4.5); f(ax, 3.17, 5.0, 3.77, 7.4, "szafa", 4.5); f(ax, 1.2, 7.7, 3.0, 8.25, "biurko", 4.5)
        f(ax, 4.0, 0.2, 4.45, 3.5, "regaly - biblioteka", 4.2); f(ax, 6.2, 1.2, 8.6, 2.1, "sofa"); f(ax, 9.3, 0.8, 11.6, 3.0, "strefa\nnauki/gier", 4.5)
    ...
    if kond == "P2":
        f(ax, 0.2, 1.0, 2.0, 3.1, "lozko\n180x200", 4.8); f(ax, -0.7, 3.9, 1.8, 4.5, "komoda", 4.5)
        f(ax, 3.75, 0.2, 4.35, 4.6, "szafy", 4.2); f(ax, 5.45, 0.2, 6.05, 3.0, "szafy", 4.2)
        ...
        f(ax, 9.0, 0.3, 11.6, 1.1, "biurko", 4.5); f(ax, 11.3, 1.6, 11.895, 4.5, "regal", 4.2)
...
def roofs_below(ax, kond):
    ...
    if kond in ("P1", "P2"):
        ...
        ax.text(15.5, 4.2 if kond == "P1" else 4.2, "DACH ZIELONY GARAZU\n(ekstensywny, nieuzytkowy)\nattyka +3,85", fontsize=6.5,
    ...
    if kond == "P2":
        ...
        ax.text(1.7, 7.0, "dach P1 (+6,50)\nzwirowy, PV", fontsize=6, ha="center", va="center", color="#444", zorder=3)
...
def overhangs(ax, kond):
    ...
    if kond == "P0":
        ...
        ax.text(5.0, -1.05, "okap ST1 (linia E) +2,80 - wysuniecie 1,00 m", fontsize=5.5, ha="center", va="center", color="#444")
        ax.text(-1.05, 2.4, "okap zach. 1,50 m", fontsize=5.5, rotation=90, ha="center", va="center", color="#444")
        ax.text(10.9, 9.62, "daszek nad wejsciem", fontsize=5.3, ha="center", va="center", color="#444")
        ...
        ax.text(4.9, -3.4, "TARAS OGRODOWY -0,05 (deska kompozytowa / plyty)  61,7 m²", fontsize=6.5, ha="center", va="center", color="#6b5b3e")
        ax.add_patch(Rectangle((9.80, 8.70), 2.50, 1.30, facecolor="#ececec", edgecolor="#999", lw=0.5, zorder=0.4))
        ax.text(11.05, 9.25, "podest -0,02 / 1 stopien", fontsize=4.8, ha="center", va="center", color="#555", zorder=1)
    if kond == "P1":
        ...
        ax.text(8.1, -0.8, "RAMA C: plyta dolna +3,65/+3,85 (linia D), plyta gorna +5,35/+5,55, wysuniecie 1,00 m", fontsize=5.5,
                ha="center", va="center", color="#333", zorder=3)
        ax.text(-1.05, 2.4, "okap ST1 ponizej", fontsize=5.3, rotation=90, ha="center", va="center", color="#555", zorder=3)
    if kond == "P2":
        ...
        while x <= L["x"][1] + 1e-6:
            ...
            x += 0.36
        ax.text(5.5, -0.95, "lamele pionowe 40x80 co 12 cm, odsuniete 15 cm od lica (drewno termo)", fontsize=5.5, ha="center",
...
def dims(ax, kond):
    if kond == "P0":
        ...
        D.dim_chain_v(ax, [M.OSIE_Y[k] for k in ("1", "2", "3", "4", "5")], 20.3, fs=6)
        D.dim_v(ax, -0.30, 9.30, 21.0, "9,60", fs=6.5, left=False)
        ...
        D.dim_h(ax, -0.30, 12.30, -5.7, "12,60 (bryla B)", fs=6.5, ext_from=-0.30)
        D.dim_chain_h(ax, [-0.30, 3.60, 4.10, 11.10, 12.60], -4.3, fs=5.5)
        ax.text(7.6, -3.95, "boks C: 3 x 2,33", fontsize=5.3, ha="center")
        D.dim_chain_v(ax, [M.OSIE_Y[k] for k in ("1", "3", "4")], 13.5, fs=6)
        D.dim_v(ax, -0.30, 8.70, -3.9, "9,00", fs=6.5)
    else:
        D.dim_chain_h(ax, [M.OSIE_X[k] for k in ("A'", "A", "B", "C", "D", "E")], -5.0, fs=6)
        D.dim_h(ax, -1.30, 12.30, -5.7, "13,60 (bryla A)", fs=6.5, ext_from=-1.30)
        D.dim_h(ax, -2.40, 12.60, -4.3, "15,00 (plyta ST3)", fs=6.0)
        D.dim_chain_v(ax, [M.OSIE_Y[k] for k in ("1", "3", "4")], 13.5, fs=6)
        D.dim_v(ax, -1.30, 5.10, -3.9, "6,40 (A)", fs=6.2)


INFO = {
    "P0": ("W2 - RZUT PARTERU P0   (+-0,00 = 101,65 m n.p.m.; posadzka P0 = 0,00; h w swietle 2,80 m)",
           "Konstrukcja: sciany nosne silikat 18 cm w osiach A, B, C, D, E, F, 1(strefa gosp.), 3, 4, 5 - ciagle przez P0-P1 (P2: A, E, 1, 3, B, C, D). "
           "Fasada pd. (E): przeszklenie 5 kwater na slupach stalowych SL1-SL4 + belka B1 25x105 (+2,80...+3,85). "
           "Strop ST1 20 cm, rozpietosci 4,80 / 3,60 m (N-S); garaz: plyta 24 cm, rozp. 6,40 m (E-W)."),
    "P1": ("W2 - RZUT I PIETRA P1   (posadzka +3,15; h w swietle 2,80 m)",
           "Bryla B 12,60 m; boks C (3 kwatery 2,33 m, parapet 0,70 z dolna czescia stala VSG do 0,85) w ramie wysunietej 1,00 m. "
           "Pokoje dzieci od zachodu (elewacja pd. bryly B pelna jak w szkicu). Pion SI (kanalizacja K1 + kanaly rekuperacji) nad pionem P0 i pod P2."),
    "P2": ("W2 - RZUT II PIETRA P2   (posadzka +6,30; h w swietle 2,80 m)",
           "Bryla A 13,60 m w lamelach, wspornik 1,00 m na zachod (sciany-tarcze zelbetowe w osiach 1 i 3 na odc. A'-B'), plyty ST2/ST3 wysuniete 1,00-1,10 m. "
           "Nadbudowa klatki schodowej i lazienki rodzicow nad osiami B-D (niewidoczna od poludnia). Wyjscie na dach: klapa 0,9x0,9 w pom. techn. 2.06."),
}
...
def plan(kond):
    ...
    columns(ax, kond)
    shafts(ax, kond)
    offs = {"0.06": (3.0, 1.0), "0.07": (4.75, 5.55), "0.05": (7.82, 6.2), "0.04": (6.7, 7.7), "0.02": (10.9, 5.6),
            "0.03": (9.07, 7.05), "0.13": (15.2, 5.0), "1.01": (1.9, 4.17), "1.02": (8.0, 2.9), "1.06": (7.26, 7.7),
    ...
    D.axes_bubbles(ax, names_x, names_y, -6.6, 11.5, -4.7, 21.8 if kond == "P0" else 14.6)
    ...
    pu = sum(M.room_area(r) for r in M.POMIESZCZENIA if r["kond"] == kond and r["kat"] != "garaz")
    txt = f"PU kondygnacji (bez garazu): {D.fmt(pu)} m²"
    if kond == "P0":
        g = next(r for r in M.POMIESZCZENIA if r["id"] == "0.13")
        txt += f"   |   garaz: {D.fmt(M.room_area(g))} m²"
    fig.text(0.02, 0.03, txt + "   |   wymiary w m, lica wykonczone; osie = srodek warstwy konstrukcyjnej   |   SI - szacht instalacyjny (pion K1 + wentylacja), K2 - pion kanalizacyjny pralni/WC",
