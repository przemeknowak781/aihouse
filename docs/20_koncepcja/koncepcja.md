...
| powierzchnia biologicznie czynna | 1 270,23 m² (79,39 %); rezerwa 50 % dachu zielonego 29,53 m² | ≥ 800 m² (50 %) | ✓ |
| intensywność zabudowy (Σ brutto kondygnacji / działka) | 0,248 | 0,05–0,80 | ✓ |
| kubatura brutto | 1 354,4 m³ | — (> 1000 m³ → PWP, W-190) | — |
| wysokość zabudowy (upzp art. 2 pkt 30 lit. a; `lamela.wskazniki`): najwyższy punkt 10,200 (wyrzutnia wentylacji (dachowa, szczyt urządzenia)) − średnia z min./maks. terenu na obwodzie (-0,360 / -0,172 → -0,266; niższa z rzędnych istn./proj. — D-15); informacyjnie od t_min: 10,56 m | **10,47 m** | ≤ 11,00 m (rezerwa → 10,70) | ✓ |
...
| taras T2 | 17,50 | 12,70 | 41,75 | 0,95 |
| taras T3 | 19,90 | 10,90 | 31,40 | 11,60 |
...
### 15.3 Weryfikacja V2 (fizyka, woda, instalacje)

| Uwaga V2 | Decyzja | Podstawa |
|---|---|---|
| N-1 — wyrzutnia z wylotem poziomym za nisko względem punktów w promieniu 10 m | **przyjęta.** Wyrzutnia na D1 z **wylotem pionowym** (ust. 7 dotyczy wylotu poziomego); **czerpnia przeniesiona na dach D3** (≥ 0,40 m nad powierzchnią przy maks. klinie, ≥ 6 m od K1): czerpnia–wyrzutnia ≥ 6 m i wyrzutnia ≥ 1,0 m ponad czerpnią — ust. 10 spełniony także w brzmieniu dosłownym („przy czym…”); `audyt_wt.py`: nowa kontrola ust. 7 (wylot poziomy) i ust. 10 dosłownie, czerpnia i wyrzutnia na różnych dachach. Kanał czerpny po D3 na podporach, przejście szczelne przez ścianę pn. bryły A (WZ-15). Pole PV D1 bez strefy czerpni — o 1 moduł więcej na D1, razem nadal 15 × 430 Wp. | WT §152 ust. 4, 7, 10–13 (Dz.U. 2022 poz. 1225 — tekst sprawdzony w ELI); W-166, W-167 |
| N-2 — do wysokości wchodziła dolna krawędź wlotu | **przyjęta.** `czerpnia_z_top`, `wyrzutnia_z_top` — maks. wysokość urządzeń jako wymaganie; `lamela.wskazniki` bierze max(z, z_top). **H zabudowy 10,47 m** (szczyt wyrzutni +10,20) ≤ 10,70 (rezerwa W-033) ≤ 11,00 | upzp art. 2 pkt 30 lit. a; W-033 |
| N-3, N-4 — cokół/spadki na TIN | patrz 15.2 (V1-02) — cokół min 0,316 m, spadki ≥ 3,0 % | brief §9 pkt 4, 6; W-019 |
| N-5 — hydroizolacja posadzki garażu poniżej nawierzchni w progach BR1/DZ2 | **przyjęta.** `otwory[O0-07, O0-08].prog`: belka progowa z betonu wodoszczelnego na PF2 do rzędnej posadzki, membrana SBS wywinięta na belkę i połączona z uszczelnieniem cokołu ≥ 0,15 m nad nawierzchnią, XPS i jastrych kończą się na belce; `przegrody.POD-G.uwagi`; korytko OL-7 w progu bramy, OL-5 przy DZ2. Wymiarowanie belki — etap BO (element na PF2) | brief §9 pkt 1, 4; WT §315–317; DIN 18533-1 (pomocniczo) |
| N-6 — WZ-09a | patrz 15.2 (V1-01) | — |
| N-7 — rynny ukryte bez przelewów; bilans PL-3 → PL-2 | **przyjęta.** Każda rynna ukryta (PL-E, PL-DA, PL-2, PL-3, PL-D): przelewy awaryjne — obniżenie blendy (rzygacz 0,15 × 0,05 m) z dnem na krawędzi płyty = górna krawędź korytka, przy wylotach i na końcach biegów (woda nie cofa się pod ścianę/próg); `rzedna_wywiniecia_przy_scianie`, `Q_rynny_min_ls` (= F_R·r·A / liczba wylotów — wymaganie dla systemu rynny, PT-IS). Rury PL-3 (RS11/RS12) przez PL-2 w tulejach, trójnikiem do RS10/RS7 — rynna PL-2 odbiera tylko własną wodę. `deszczowa.py`: dopływy między polami (`do` = pole / rura), przepływ łączny w rurach wspólnych, kontrola przelewów rynien (F_R·Q). Wynik: RS10 = PL-2 (NE) + RS11, RS7 = PL-E + PL-2 + RS12, dopływ na D4 — 0 niespełnionych | brief §9 pkt 3; PN-EN 12056-3 [NZW]; W-142 |
| N-8 — K-13 | patrz 15.1 | — |
...
| Pozycja | Decyzja / wynik | Podstawa |
|---|---|---|
| V3 — kolizje (skrypt: rysunek każdego mebla i skrzydła drzwi logiką `lamela.views.plan`, przecięcia mebel↔mebel, mebel↔ściana, mebel↔łuk, łuk↔łuk; 62 elementy, 23 łuki) | przed poprawkami: moduł hydrauliczny PC, zasobnik c.w.u. i bufor (0.12) oraz centrala (2.07) **rysowane w ścianie** (symbole z punktem w środku / innej konwencji obrotu), wyspa z hokerami (V1-05). Poprawki: `views.plan.draw_furniture` — `zasobnik`, `rekuperator`, `pompa_ciepla` z `xy` na licu i `obrot` od ściany (jak pozostałe meble); centrala odsunięta 0,15 m od ściany (króćce ODA/ETA). **Wynik: 0 kolizji** (stelaże WC w zabudowie — linia kreskowa, oznaczenie umowne). Rzuty `podglad_modelu.py` P0–P2 obejrzane: przejścia, strefy przed meblami i ciągi bez zmian względem 14.1 | W-059, W-060; ergonomia (14.1) |
| BRAKI PZT | uzupełnione: PC-JZ `strefa_r`, `wym` (przykładowe [DO UZUPEŁNIENIA wg DTR]), sąsiedzi `kondygnacje`/`funkcja` (dane fikcyjne mapy), furtka `otwieranie` (do wewnątrz — WT §42). Pozostają dane zewnętrzne: zjazd (zezwolenie zarządcy), rzędne sieci, studzienki KD, obwody pni, zbiornik (DTR), wskaźniki MPZP w modelu (tekst uchwały czytany przez tomy) | RPB §14–15 |
